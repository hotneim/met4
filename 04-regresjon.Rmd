...
df$eps_norm <- rnorm(100)
df$eps_lin <- rnorm(100, sd = df$sigma_lin)
df$eps_sin <- rnorm(100, sd = df$sigma_sin)
eps_ar_base = rnorm(102)
df$eps_ma2 = eps_ar_base[3:102] + 0.9 * eps_ar_base[2:101] + 0.8 * eps_ar_base[1:100]
```

a. 
```{r, echo = FALSE}
plot(df$eps_norm, ylab = "Residualer", xlab = "Prediksjon")
```
b.
```{r, echo = FALSE}
plot(df$eps_lin, ylab = "Residualer", xlab = "Prediksjon")
```
c. 
```{r, echo = FALSE}
plot(df$eps_sin, ylab = "Residualer", xlab = "Datapunkter (sortert i tid)")
```

d.
```{r, echo = FALSE}
plot(df$eps_ma2, ylab = "Residualer", xlab = "Datapunkter (sortert i tid)")
```
...
a. Her ser støyleddet ganske normalfordelt ut. Det betyr at antagelsene er oppfylt.

b. Støyen er symmetrisk om null, men kan se ut som den øker. Altså er det brudd på antagelsen om normalfordeling. Økningen ser ut til å være jevn, og en log-transformasjon kunne i dette tilfellet vært til hjelp.
...
#### Oppgave 2 
Denne oppgaven følger samme premiss som oppgave 1, men handler om kolinearitet. Last ned datasettene [1](genererte_dataset/colinearity_1.RData) og [2](genererte_dataset/colinearity_2.RData), hvor y er responsvariabel og forklaringsvariable er navngitt x. 
...
saveRDS(df_colin1, file = "genererte_datasett/colinearity_1.RData")
set.seed(4)
...
saveRDS(df_colin2, file = "genererte_datasett/colinearity_2.RData")
```
...
Di kan gjøre de samme antakelsene om datasettet som i [den tilsvarende oversikten i forrige modul](#relevant-r-testing): Datasettene er inneholdt i Excel-filer (`.xls` eller `xslx`) eller `.csv`-filer, som kolonner av variabler, med variabelnavn i første rad.
