...
<!--chapter:end:06-avansert-regresjon-og-maskinlaering.Rmd-->
...
I denne dataøvingen skal vi predikere strømprisen. Resultatene fra øvingen vår trengs som imput in **Case 3** i BED4, som for mange studenter går parallelt med MET4. Hvis du ikke tar BED4 dette semesteret, så går det helt fint også. Dataøvingen står fint på egne bein, og du kan uansett komme tilbake til disse resultatene hvis du for eksempel skal ta BED4 på et senere tidspunkt.
...
[... **en eller annen motivasjon i BED4-språk om hvorfor vi er interessert i prognoser av den neste dags prisserie**...]

Vi konsentrerer oss om NO3 i første omgang, og kikker raskt på datasettet ved å plotte tidsrekken direkte med `plot()`-funksjonen:
...
For å kunne gjøre en vurdering av hvor gode prediksjonene våre er, tar vi nå å deler opp datasettet vårt i to, der vi tar ut de siste 24 timene som vi kan bruke til å evaluere prediksjonene. Vi kaller disse to delene `no3_train`, som er den lange delen som vi skal bruke til å estimere en modell (*trene* en modell), og `no3_test` som er de siste 24 timene som vi skal bruke til å teste etterpå om modellen duger. 
...
Du kan nå dobbelsjekke at du har fått ut en enkelt dag i `no3_test` ved å kjøre `plot(no3_test)`.

### Oppgave 3: Hent ut sesong og trend

Vi har lært at et første steg i tidsrekkeanalyse er å hente ut eventuelle sesong og trendkomponenter. Vi har også sett at det er en enkel funksjon i R som kan gjøre dette for oss, nemlig `stl()`. Vi ønsker å bruke det meget praktiske `autoplot`

```{r}
dekomponert <- stl(no3, s.window = "periodic")
autoplot(dekomponert)
plot(dekomponert2, xlim = c(220, 260))
```
...
<!--chapter:end:08-seminaroppgaver.Rmd-->
...
<!--chapter:end:09-Eksamensoppgaver.Rmd-->
